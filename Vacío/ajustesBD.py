...
BD[0] = BD[0].iloc[1:].reset_index(drop=True)
BD[2] = BD[2].iloc[1:].reset_index(drop=True)


Parametros, Errores = [], []
Chis, Pvals = [], []
for i in (0,2):
    pop, cov = curve_fit(bombeo, BD[i].index.values, BD[i]["Presión"].values, sigma = error(BD[i]) , p0=[0.00156/4.5e-3, BD[i]["Presión"].loc[0], np.min(BD[i]["Presión"].values)], absolute_sigma=True)
    Parametros.append(pop)
    Errores.append(np.sqrt(np.diag(cov)))
    chi, pv, nu = chi2_pvalor(BD[i].index.values, BD[i]["Presión"].values, error(BD[i]), bombeo(BD[i].index.values, *pop), 2)
    Chis.append(chi/nu)
    Pvals.append(pv)
    # residuos(bombeo, pop, BD[i].index.values, BD[i]["Presión"].values, error(BD[i]), grafico = True)
...
ejex = np.linspace(0, 70, 10000)

# 1,3
for i in range(len(Parametros)):
    ind = (0,2)
    label = (
    rf'$S/V = {Parametros[i][0]:.1f} \pm {Errores[i][0]:.1f}$' + '\n' +
    rf'$\chi^2 = {Chis[i]:.1f}$' + '\n' +
    rf'$pval = {Pvals[i]:.1f}$'
    )
    ...
    plt.fill_between(BD[ind[i]].index, BD[ind[i]]["Presión"] - error(BD[ind[i]]), BD[ind[i]]["Presión"] + error(BD[ind[i]]), alpha=0.3)
    # plt.axhline(y = 2.7e-5, color = "k", linestyle = "--", label = "saturación", alpha=0.5)
    plt.plot(BD[ind[i]].index, BD[ind[i]]["Presión"],".k", label="Medición BD")
    ...
    plt.xlim(0,np.max(BD[ind[i]].index)*1.2)
